...
void aruco_wrapper::getpos(cv::Mat Rvec, cv::Mat Tvec, float position[3])
{
  ...
  pos = -rmat.inv() * Tvec.t();
  position[0] = pos.at<float>(0,0) - offset_x_;
  position[1] = pos.at<float>(0,1) - offset_y_;
  position[2] = pos.at<float>(0,2) - offset_z_;
}

void aruco_wrapper::geterr_(aruco::Marker marker, cv::Mat Rvec, cv::Mat Tvec, float err_[3])
{
  // get 3Dinfo from marker map
  aruco::Marker3DInfo a = MM.getMarker3DInfo(marker.id);
  ...
  // Rotation Matrix
  cv::Mat rmat, dr_drmat;
  cv::Rodrigues(Rvec, rmat, dr_drmat);
  Matrix3f R;
...
void aruco_wrapper::geterr(vector<aruco::Marker> v_m, cv::Mat Rvec, cv::Mat Tvec, float err[3])
{
  float dst[3] = {0, 0, 0};
...
void aruco_wrapper::getquaternion(cv::Mat Rvec, float quat[3])
{
  cv::Mat rmat;
  cv::Rodrigues(Rvec, rmat);
...
bool aruco_wrapper::checkmarker(vector<aruco::Marker> v_m)
{
...
bool aruco_wrapper::checkpos(float position[3], float dt)
{
  if (position[2]>0||position[2]<-50||isnan(position[2])||isinf(position[2])||dt > 1000000) {
    aruco::MarkerMapPoseTracker temp;
...
bool aruco_wrapper::MarkerUpdate(cv::Mat img)
{
  gettimeofday(&tv, NULL);
  packet.timestamp = (tv.tv_sec % 1000) * 1000000 + tv.tv_usec;

  vector<aruco::Marker> v_m;
  ...
  if (judge) {
    ...
    cv::Mat Rvec = MMPT.getRvec();
    cv::Mat Tvec = MMPT.getTvec();
...
{
  cout << "******************************************" << endl;
  cout << "Timestamp: " << unsigned(packet.timestamp) << endl;
...
{
  fout << unsigned(packet.timestamp) << ",";
